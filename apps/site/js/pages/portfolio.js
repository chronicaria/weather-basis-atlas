...
/** Say what is wrong once, in the page's own words, instead of once per row. */
const CSV_COLUMNS = { row_id: 'Row', kind: 'Kind of holding', entity_id: 'Location and index', amount: 'Amount', budget: 'Baseline degree days', candidate_id: 'Hedge candidate', payoff_kind: 'Payoff', strike: 'Strike', multiplier: 'Dollars per degree day', unit_cost: 'Unit cost', units: 'Units', currency: 'Currency' };
function csvProblem(errors) {
  const missing = /Missing required columns: (.+)\.$/.exec(errors[0] || '');
  if (missing) return `It needs a header row with these columns: ${missing[1].split(', ').map((key) => `${CSV_COLUMNS[key] || key} (${key})`).join(', ')}. Download the template above for a file that already has them.`;
  const rows = errors.map((text) => /^Row (\d+)/.exec(text)?.[1]).filter(Boolean);
  if (rows.length) return `${rows.length} row${rows.length === 1 ? '' : 's'} could not be read (${rows.slice(0, 8).join(', ')}${rows.length > 8 ? ', …' : ''}). Each row needs a row name, a kind of holding, a location and index such as 31109:HDD-01, and an amount in dollars per degree day.`;
  return errors.join(' ');
}
...
export function mountPortfolio({ bootstrap, scenario, loadObject, countyFor, pageLink }) {
  ...
  /* ---- Import / export ------------------------------------------------ */
  importInput.addEventListener('change', async () => { const file = importInput.files?.[0]; if (!file) return; const text = await file.text(); if (file.name.endsWith('.json')) { try { const decoded = JSON.parse(text); if (decoded.decision_id) { if (decoded.decision_id !== await decisionIdentity(decoded)) throw new Error('identity mismatch'); acceptedDecision = decoded; localBook = decoded.request?.book || decoded.request || localBook; holdingsDirty = false; setExports(true); markCard(null); bookSummary.replaceChildren(); displayedResultStale = false; renderResult(acceptedDecision); bookStatus.textContent = 'Decision record restored without recalculation; its identity checked out. The result is shown in step 4.'; } else { const rows = Array.isArray(decoded.holdings) ? decoded.holdings : null;
          if (!rows) throw new Error('not a book');
          const kept = rows.filter((row) => row && typeof row === 'object' && row.entity_id).slice(0, MAX_IMPORT_ROWS);
          if (!kept.length) throw new Error('no usable holdings');
          localBook = { ...decoded, holdings: kept }; holdingsDirty = true; markCard(null); bookSummary.replaceChildren();
          const dropped = rows.length - kept.length;
          bookStatus.textContent = `${kept.length} holding row${kept.length === 1 ? '' : 's'} loaded. It stays in this browser and is not published or shared.${dropped > 0 ? ` ${dropped} row${dropped === 1 ? ' was' : 's were'} left out: ${rows.length > MAX_IMPORT_ROWS ? `this page takes at most ${MAX_IMPORT_ROWS} rows` : 'they have no location and index'}.` : ''}`; } renderHoldings(localBook.holdings || []); renderCandidateChoices(); } catch { bookStatus.textContent = 'That file is not a book or a decision record from this page. A book is a JSON file with a list of holdings; use “Save book (JSON)” to see the shape it expects.'; } } else { const parsed = parseHoldingsCsv(text); if (parsed.errors.length) { bookStatus.textContent = `The CSV was not accepted. ${csvProblem(parsed.errors)}`; return; } localBook.holdings = parsed.rows; holdingsDirty = true; renderHoldings(parsed.rows); renderCandidateChoices(); bookStatus.textContent = `${parsed.rows.length} holding row${parsed.rows.length === 1 ? '' : 's'} imported as written, without unit conversion or dropping locations.`; } document.dispatchEvent(new Event('wba-book-change')); });
