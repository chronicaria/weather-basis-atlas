...
the sealed site leaf is still being assembled.

When an explicit verified bundle exists, serve only that bundle on loopback:

```bash
uv run wba v2 release verify --bundle build/releases/<release-id>
uv run wba v2 serve --bundle build/releases/<release-id>
```
